...
describe('SearchPromptsUseCase', () => {
  const input: Prompt[] = [
      { id: '1', title: 'AI Prompt 1', content: 'Content 1', createdAt: new Date(), updatedAt: new Date() },
      { id: '2', title: 'AI Prompt 2', content: 'Content 2', createdAt: new Date(), updatedAt: new Date() },
    ];
    
  const repository: PromptRepository = {
    findMany: async () => input,
    searchMany: async (term) => input.filter(
      (prompt) => 
