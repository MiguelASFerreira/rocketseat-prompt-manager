...
describe('PromptForm', () => {
  ...
  it('deve criar um novo prompt com sucesso', async () => {
    ...
    await user.type(contentInput, 'Este é o conteúdo do meu prompt de teste.');

    const submitButton = screen.getByRole('button', { name: /salvar/i });
    await user.click(submitButton);

    expect(createActionMock).toHaveBeenCalledWith({
      title: 'Meu Prompt de Teste',
      content: 'Este é o conteúdo do meu prompt de teste.',
    });
  ...
  it('deve atualizar um prompt existente com sucesso', async () => {
    updateActionMock.mockReturnValue({
      success: true,
